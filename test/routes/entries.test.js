...
test('Nao deve inserir lancamento sem tipo', () => {
  return request(app)
    .post(MAIN_ROUTE)
    .send({
      name: 'Lanc #1',
    ...
    .then((result) => {
      expect(result.status).toBe(400);
      expect(result.body.error).toBe('Tipo é obrigatório');
    });
});
...
test('Nao deve inserir lancamento sem ID do usuario', () => {
  return request(app)
    .post(MAIN_ROUTE)
    .send({
      name: 'Lanc #1',
    ...
    .then((result) => {
      expect(result.status).toBe(400);
      expect(result.body.error).toBe('ID do usuário é obrigatório');
    });
});
